...
实验内容：
- 使用双向LSTM（BiLSTM）同时从前向和后向捕捉时序特征
- 结合周期性时间特征工程
...
def main():
    ...
    plot_predictions(actuals, predictions, "双向LSTM+时间特征", 
                    os.path.join(results_dir, "step4_predictions.png"))
    plot_scatter(actuals, predictions, "双向LSTM+时间特征",
                os.path.join(results_dir, "step4_scatter.png"))
    
    generate_report(metrics, "步骤4: 双向LSTM + 时间特征",
                   os.path.join(docs_dir, "step4_report.md"))
