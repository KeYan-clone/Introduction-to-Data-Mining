...
实验内容：
- 在LSTM基础上引入周期性时间特征（小时、星期、月份的sin/cos编码）
- 特征维度：21个原始特征 + 6个时间特征 = 27维
- 目的：利用天气的周期性规律提升预测精度
- 时间特征：
  * hour_sin/cos: 捕捉24小时周期
  * day_sin/cos: 捕捉一周周期
  * month_sin/cos: 捕捉季节性周期
"""
...
def main():
    ...
    # 准备数据（包含时间特征），先创建序列再按训练集拟合 scaler
    ...
    plot_predictions(actuals, predictions, "LSTM+时间特征", 
                    os.path.join(results_dir, "step3_predictions.png"))
    plot_scatter(actuals, predictions, "LSTM+时间特征",
                os.path.join(results_dir, "step3_scatter.png"))
    
    generate_report(metrics, "步骤3: LSTM + 时间特征",
                   os.path.join(docs_dir, "step3_report.md"))
